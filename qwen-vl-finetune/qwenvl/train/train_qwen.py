...
class OmniTrainer(Trainer):
    def __init__(self, *args, stage="stage1", **kwargs):
        super().__init__(*args, **kwargs)
        self.stage = stage # "stage1" or "stage2"

    def compute_loss(self, model, inputs, return_outputs=False):
        # 这里的 inputs 已经被 DataCollator 处理过了
        
        # 传入 stage 参数
        loss, loss_dict, pred_traj = model(
            **inputs,
            stage=self.stage
        )
        ...
        if self.state.global_step % 10 == 0:
            print(f"Step {self.state.global_step}: {loss_dict}")

        return (loss, {'pred_traj': pred_traj}) if return_outputs else loss
