...
class RossStableDiffusionXOmni(nn.Module):
    def __init__(
        ...
        unet_path, 
        subfolder="scheduler",  # 直接指定 scheduler 文件夹
        local_files_only=True   # 强制本地
        )
        self.target_h = 36
        self.target_w = 96

    def forward(self, z, target, z_a=None,encoder_hidden_states=None):
        ...
        #     # 正常情况：使用传入的 Action Query
        #     prompt_embeds = encoder_hidden_states
        if encoder_hidden_states is not None:
            # Case 1: 正常训练，传入了 LLM 的 Action Embeds
            pass
        else:
            # Case 2: 没传参数，加载默认的 Negative Prompt
            print("⚠️ Loading Negative Prompt (No Action Query passed)")
            encoder_hidden_states = torch.load(self.negative_prompt_path).to(target.device).to(target.dtype)
            encoder_hidden_states = encoder_hidden_states.repeat(bsz, 1, 1)
            # prompt_embeds = torch.load(self.negative_prompt_path).to(target.device).to(target.dtype)
        ...
        # _, _, z_h, z_w = z.shape
        if z.shape[2] != self.target_h or z.shape[3] != self.target_w:
            z = F.interpolate(z, size=(self.target_h, self.target_w), mode="bilinear").contiguous()   

        z = z.permute(0, 2, 3, 1) 
        z = self.ln_pre(z)          # 把 [-27, 18] 归一化到 [-2, 2]
        z = z.permute(0, 3, 1, 2) 
        _, _, z_h, z_w = z.shape
    ...
    def inference(
        ...
        height=288, # 🔥 修复：默认设为训练分辨率 288
        width=768,  # 🔥 修复：默认设为训练分辨率 768  # not supported, as training do not have cfg
    ):
        ...
        if encoder_hidden_states is not None:
            prompt_embeds = encoder_hidden_states
             ...
             # 如果 inference 没传，默认用负面/空 prompt 是合理的（无条件生成）
             # 这里不报 warning，或者你可以选择报 info
